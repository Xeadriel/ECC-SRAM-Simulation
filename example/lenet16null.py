...
def main_work_flow():
   ...
   # print(dict)
         ...
         for bt in bt_data:
            ...
            # p.dtype = 'int8'
                if bt ==0:
                    ...
                    fc2_q_c = EA.error_Apply(p, 4, bet, [14,15], 0)
                # print([bt])
                # print(key)
                # print(fc2_q_c)
            ...
            # p.dtype = 'int8'
                if bt ==0:
                     ...
                     fc2_q_c = EA.error_Apply(p, 2, bet, [14,15], 0)
                # fc2_q_c_cuda = fc2_q_c.to(device)
            # print(fc2_q_c)
                dict[key] = torch.tensor(fc2_q_c).to(device)
     ...
     # 保存每次的数据到txt文件
     with open(f'16_bi_data_run_{run}.txt', 'w') as f:
...
# 计算平均准确性
   ...
   with open(f'16_bi_data_avg.txt', 'w') as f:
       f.write("\t".join(map(str, bet_data)) + "\n")
       for i, bt in enumerate(bt_data):
