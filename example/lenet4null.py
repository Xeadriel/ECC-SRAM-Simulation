...
def main_work_flow():
   ...
   torch.save(net.state_dict(), "lenet5_dense_quantized_4.pt")

    # step4, used the sparse_quantized model
   print("Evaluate sparse quantized model:")
   net = LeNet5(4)
   net = net.to(device)
   net.load_state_dict(torch.load("lenet5_dense_quantized_4.pt"), strict=False)
   net.quan_mode()
   acc(net, test_data, test_label)
   ...
   # print(dict)
     ...
     for bet in bet_data:
         ...
         for bt in bt_data:
            net = LeNet5(4)
            net = net.to(device)
            dict = torch.load("lenet5_dense_quantized_4.pt")
            for key in ("conv1.quan_weight", "conv2.quan_weight", "conv3.quan_weight"):
                p_num = dict[key].int()
                p = p_num.cpu()
                p.numpy()
            # p.dtype = 'int8'
                fc2_q_c = EA.error_Apply(p, 4, bet, [], 0)
                # print([bt])
                # print(key)
                # print(fc2_q_c)
            ...
            for key in ("fc1.quan_weight", "fc2.quan_weight"):
                ...
                p.numpy()
            # p.dtype = 'int8'
                fc2_q_c = EA.error_Apply(p, 2, bet, [], 0)
                # fc2_q_c_cuda = fc2_q_c.to(device)
            # print(fc2_q_c)
                dict[key] = torch.tensor(fc2_q_c).to(device)
     ...
     # 保存每次的数据到txt文件
     with open(f'4null_data_run_{run}.txt', 'w') as f:
...
# 计算平均准确性
   ...
   with open(f'4null_data_avg.txt', 'w') as f:
       f.write("\t".join(map(str, bet_data)) + "\n")
       for i, bt in enumerate(bt_data):
